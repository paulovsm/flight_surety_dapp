...
contract FlightSuretyData {
    ...
    modifier requireAirlinesRegistered(address[] calldata airlineAddresses) {
        for (uint i = 0; i < airlineAddresses.length; i++) {
            ...
            require(
                airline.airlineAddress != address(0),
    ...
    modifier requireMinimumConsensus(address[] calldata airlineAddresses) {
        if (numAirlines < 4) {
            ...
            require(
                airlineAddresses.length >= numAirlines / 2,
                "Minimum consesus not met"
            );
        }
        _; // All modifiers require an "_" which indicates where the function body will be added
    }
    ...
    modifier requireMinimumFunding(address airlineAddress) {
        require(
            airlines[airlineAddress].funding >= 10 ether,
            "Airline not at minimum funding"
        );
        _; // All modifiers require an "_" which indicates where the function body will be added
    }
    ...
    function getBalanceDue(address _passenger) public view returns (uint256) {
        ...
        if (
            passenger.passengerAddress == _passenger &&
     ...
     *
     */
    function pay(address payable _passenger) public payable {
    ...
    function getFlight(string calldata flightId)
        public
        view
